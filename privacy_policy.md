Privacy Policy - Fanta F1
Effective Date: 29 January 2026

Last Updated: 29 January 2026

Introduction
Fanta F1 ("we", "our", "us") respects your privacy and is committed to protecting your personal data. This Privacy Policy explains how we collect, use, and share information when you use our mobile application Fanta F1 (the "App").

Information We Collect
1. Information You Provide
When you use Fanta F1, you may provide us with:

Account Information: Email address, display name
Game Data: League memberships, team formations, driver selections, points, standings
Profile Information: Username, preferences
2. Automatically Collected Information
We automatically collect certain information when you use the App:

Device Information: Device type, operating system version, unique device identifiers
Usage Data: App features used, interaction patterns, crash reports
Advertising Data: Ad impressions, clicks (via Google AdMob)
3. Information from Third Parties
We use the following third-party services that may collect information:

Supabase: Backend database and authentication service
Google AdMob: Advertising platform
RevenueCat: In-app purchase management
How We Use Your Information
We use the collected information for:

Providing App Functionality: Account management, league participation, score calculation, leaderboards
Authentication: Secure login and account access via Supabase
In-App Purchases: Processing premium subscriptions via RevenueCat
Advertising: Displaying relevant ads via Google AdMob (for free users)
App Improvement: Analytics, bug fixes, feature development
Communication: App updates, announcements, customer support
Data Storage and Security
Hosting: Your data is stored on Supabase servers (cloud infrastructure)
Location: Data may be stored in servers located in the European Union or United States
Security: We implement industry-standard security measures including encryption and secure authentication
Retention: We retain your data as long as your account is active or as needed to provide services
Third-Party Services
Supabase
Our backend and authentication provider. See Supabase Privacy Policy.

Google AdMob
Displays advertisements in the free version of the App. AdMob may collect device identifiers and usage data for ad personalization. See Google Privacy Policy.

RevenueCat
Manages in-app purchases and subscriptions. See RevenueCat Privacy Policy.

Your Rights
Depending on your location, you may have the following rights:

Access: Request a copy of your personal data
Correction: Update or correct your personal data
Deletion: Request deletion of your account and personal data
Opt-Out: Disable personalized advertising (via device settings)
Data Portability: Receive your data in a structured format
To exercise these rights, contact us at: 
support@fantaf1.app
 (update with your actual email)

Children's Privacy
Fanta F1 is not intended for children under 13 years of age. We do not knowingly collect personal information from children under 13. If you believe a child has provided us with personal data, please contact us.

Changes to This Privacy Policy
We may update this Privacy Policy from time to time. We will notify you of material changes by:

Posting the updated policy in the App
Updating the "Last Updated" date
Sending an in-app notification (for significant changes)
Your continued use of the App after changes constitutes acceptance of the updated policy.

Contact Us
If you have questions about this Privacy Policy or our data practices, contact us at:

Email: 
support@fantaf1.app
 (update with your actual email)

Address: [Your Address] (optional, required for some jurisdictions)

GDPR Compliance (European Users)
If you are located in the European Economic Area (EEA), we process your data based on:

Consent: For advertising and analytics
Contract: To provide App services
Legitimate Interest: For app improvement and security
You have the right to withdraw consent at any time, lodge a complaint with a supervisory authority, and exercise all rights listed in the "Your Rights" section above.

Data Controller
For GDPR purposes, the data controller is:

Fanta F1
Giuliano Dal Pont
dalpontgiuliano@gmail.com

Last Updated: 29 January 2026

Version: 1.0
